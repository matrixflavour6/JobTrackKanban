...
export default function App() {
  ...
  const [viewMode, setViewMode] = useState<'kanban' | 'table' | 'analytics' | 'compare'>('kanban');
  ...
  return (
    <div className="min-h-screen bg-paper text-ink font-sans flex flex-col selection:bg-ledger selection:text-white">
      ...
      {(!licenseState.isAuthenticated || licenseState.isGuest) && (
        <div className="bg-gradient-to-r from-brass/10 via-orange-500/10 to-brass/10 border-b border-brass/20 px-4 py-2.5 text-xs text-brass flex items-center justify-between shrink-0 shadow-2xs">
          ...
          <button
            ...
            className="ml-3 px-3 py-1 bg-brass hover:bg-brass text-white font-semibold text-[11px] rounded-lg shadow-2xs transition-colors shrink-0 cursor-pointer flex items-center space-x-1"
          >
        ...
        {(searchQuery || selectedTag) && (
          <div className="mb-4 bg-white border border-slate-200 rounded-xl px-4 py-2.5 flex items-center justify-between text-xs shadow-2xs">
            ...
            <button
              ...
              className="text-ledger hover:text-ledger font-medium cursor-pointer"
            >
              Reset Filters
            </button>
          </div>
      ...
      {toast && (
        <div className="fixed bottom-5 right-5 z-50 bg-slate-900 text-white px-4 py-3 rounded-xl shadow-xl border border-slate-800 text-xs font-semibold flex items-center space-x-2.5 animate-bounce">
          {toast.type === 'success' && <CheckCircle className="w-4 h-4 text-brass" />}
          {toast.type === 'error' && <AlertCircle className="w-4 h-4 text-stamp" />}
          {toast.type === 'info' && <Info className="w-4 h-4 text-ledger" />}
