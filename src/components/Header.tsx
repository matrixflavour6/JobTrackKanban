...
interface HeaderProps {
  ...
  viewMode: 'kanban' | 'table' | 'analytics' | 'compare';
  setViewMode: (mode: 'kanban' | 'table' | 'analytics' | 'compare') => void;
  searchQuery: string;
    ...
    <header className="ledger-glass sticky top-0 z-20 border-b border-black/5 shadow-2xs">
      ...
      <div className="max-w-[1800px] w-full mx-auto px-4 sm:px-6 lg:px-8 py-3">
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3">
          ...
          <div className="flex items-center space-x-2 text-xs overflow-x-auto pb-1 lg:pb-0 scrollbar-none whitespace-nowrap">
            ...
            <div className="flex items-center px-3 py-1.5 rounded-full bg-plum/10 border border-plum/20 shrink-0">
              ...
              <span className="font-bold text-plum">{interviewing}</span>
            </div>
            ...
            <div className="flex items-center px-3 py-1.5 rounded-full bg-brass/10 border border-brass/20 shrink-0">
              ...
              <span className="font-bold text-brass">{offers}</span>
            </div>
          ...
          <div className="flex items-center space-x-2 shrink-0">
            <button
              ...
              className="inline-flex items-center px-3.5 py-1.5 sm:py-2 text-xs font-semibold rounded-full bg-gradient-to-r from-ledger to-indigo-600 hover:from-ledger hover:to-indigo-500 text-white shadow-2xs transition-all cursor-pointer"
            >
              <Globe className="w-3.5 h-3.5 mr-1.5 text-ledger" />
            ...
            <button
              ...
              className="inline-flex items-center px-4 py-1.5 sm:py-2 text-xs font-semibold rounded-full text-white bg-ledger hover:bg-ledger active:bg-ledger shadow-sm transition-all cursor-pointer shrink-0"
            >
        ...
        <div className="mt-2.5 pt-2.5 border-t border-black/5 flex flex-col md:flex-row md:items-center justify-between gap-2.5">
          ...
          <div className="flex items-center bg-paper-dim p-1 rounded-xl border border-ink/8 self-start md:self-auto shrink-0 shadow-inner overflow-x-auto">
            <button
          ...
          <div className="flex items-center space-x-2 flex-1 max-w-full md:max-w-md w-full">
            ...
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full pl-8 pr-7 py-1.5 bg-white/70 hover:bg-white focus:bg-white border border-black/10 focus:border-ledger rounded-full text-xs text-slate-800 placeholder-slate-400 focus:outline-hidden focus:ring-2 focus:ring-ledger/20 transition-all"
              />
              {searchQuery && (
                <button
            ...
            {allTags.length > 0 && (
              <div className="relative shrink-0">
                <select
                  ...
                  className="pl-7 pr-6 py-1.5 bg-white/70 border border-black/10 rounded-full text-xs text-slate-700 font-medium focus:outline-hidden focus:border-ledger appearance-none cursor-pointer hover:bg-white transition-all"
                >
