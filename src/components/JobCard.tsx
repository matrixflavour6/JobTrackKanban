...
export const JobCard: React.FC<JobCardProps> = ({
  ...
  const getAvatarBg = (name: string) => {
    const colors = [
      'bg-ledger', 'bg-indigo-600', 'bg-plum', 'bg-brass', 
      'bg-teal-600', 'bg-slate-700', 'bg-cyan-600', 'bg-stamp'
    ];
    let hash = 0;
  ...
  return (
    ...
    >
      ...
      <div
        className={`ledger-stamp absolute -top-2 right-3 ${
          job.stage === 'wishlist' ? 'text-slate bg-slate-soft' :
          job.stage === 'applied' ? 'text-ledger bg-ledger-soft' :
          job.stage === 'interview' ? 'text-plum bg-plum-soft' :
          job.stage === 'offer' ? 'text-brass bg-brass-soft' :
          'text-stamp bg-stamp-soft'
        }`}
      >
        {STAGES.find(s => s.id === job.stage)?.title || job.stage}
      </div>
        ...
        <div className="flex items-center space-x-2.5 min-w-0">
          ...
          <div className="min-w-0">
            <div className="flex items-center space-x-1.5 flex-wrap">
              <h3 className="text-sm font-semibold text-slate-900 truncate group-hover:text-ledger transition-colors tracking-tight">
                {job.position}
              </h3>
              {job.priority === 'high' && (
                <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold bg-stamp-soft text-stamp border border-stamp shrink-0">
                  <Flame className="w-2.5 h-2.5 mr-0.5 fill-stamp text-stamp" />
                  High
                </span>
              )}
              {job.priority === 'medium' && (
                <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold bg-brass-soft text-brass border border-brass shrink-0">
                  Med
                </span>
              )}
              {job.priority === 'low' && (
            ...
            <>
              ...
              <div className="absolute right-0 top-6 w-40 bg-white border border-slate-200 rounded-lg shadow-lg py-1 z-20 text-xs">
                ...
                {job.followUpDate && (
                  <>
                    <a
                      ...
                      rel="noopener noreferrer"
                      onClick={() => setShowMenu(false)}
                      className="w-full text-left px-3 py-1.5 hover:bg-ledger-soft text-ledger flex items-center cursor-pointer font-medium"
                    >
                      <Calendar className="w-3.5 h-3.5 mr-2 text-ledger" />
                ...
                <button
                  ...
                  className="w-full text-left px-3 py-1.5 hover:bg-stamp-soft text-stamp flex items-center cursor-pointer"
                >
                  <Trash2 className="w-3.5 h-3.5 mr-2 text-stamp" />
                  Delete Card
                </button>
              </div>
            </>
      ...
      <div className="space-y-1 my-2 text-xs text-slate-500">
        ...
        {job.salary && (
          <div className="flex items-center text-slate-700 font-medium">
            <DollarSign className="w-3 h-3 mr-1.5 text-brass shrink-0" />
      ...
      {job.interviewRounds && job.interviewRounds.length > 0 && (
        <div className="my-2 p-1.5 bg-plum-soft/70 border border-plum-soft rounded-md text-[11px] text-purple-900 flex items-center justify-between">
          <div className="flex items-center">
            <Clock className="w-3 h-3 mr-1 text-plum" />
            <span>
      ...
      {isFollowUpDue && (
        <div className={`my-2 px-2.5 py-1.5 rounded-xl text-[11px] font-medium flex items-center justify-between shadow-2xs ${
          isFollowUpToday 
            ? 'bg-brass-soft text-brass border border-brass' 
            : 'bg-stamp-soft text-stamp border border-stamp'
        }`}>
          <div className="flex items-center">
          ...
          <button
            ...
            title="Download universal .ics event for Calendar"
          ...
          >
            <Calendar className="w-3 h-3 text-ledger" />
            <span>.ics</span>
          </button>
        </div>
      )}
