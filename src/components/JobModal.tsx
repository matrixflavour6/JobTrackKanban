      ...
      <div className="bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-hidden shadow-2xl flex flex-col border border-slate-200">
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-5 text-xs text-slate-700">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="block font-semibold text-slate-800 mb-1">
                Company Name <span className="text-stamp">*</span>
              </label>
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-9 pr-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-slate-900 focus:outline-hidden focus:ring-2 focus:ring-ledger focus:bg-white"
                />
              </div>
            </div>
            ...
            <div>
              <label className="block font-semibold text-slate-800 mb-1">
                Job Position / Title <span className="text-stamp">*</span>
              </label>
              <input
                type="text"
                required
                ...
                className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-slate-900 focus:outline-hidden focus:ring-2 focus:ring-ledger focus:bg-white"
              />
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div>
              ...
              <select
                ...
                className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg font-medium text-slate-900 focus:outline-hidden focus:ring-2 focus:ring-ledger"
              >
                {STAGES.map((s) => (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 bg-slate-50/80 p-3.5 rounded-xl border border-slate-200">
            <div>
              ...
              <input
                ...
                className="w-full px-3 py-1.5 bg-white border border-slate-200 rounded-lg text-slate-900 focus:outline-hidden focus:ring-2 focus:ring-ledger"
              />
            </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-1">
                <label className="font-semibold text-slate-800 flex items-center">
                  <Calendar className="w-3.5 h-3.5 mr-1 text-brass" />
                ...
                <div className="mt-2 space-y-1">
                  <div className="flex items-center space-x-2 text-[11px]">
                    <a
                      ...
                      className="px-2 py-0.5 rounded bg-ledger-soft hover:bg-ledger-soft text-ledger font-semibold border border-ledger cursor-pointer transition-colors"
                    >
                      + Google Calendar
                    </a>
                    <a
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div>
              ...
              <input
                ...
                placeholder="https://..."
                className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-slate-900 focus:outline-hidden focus:ring-2 focus:ring-ledger"
              />
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 bg-slate-50/50 p-3.5 rounded-xl border border-slate-200/80">
              ...
              <div className="grid grid-cols-3 gap-2">
                ...
                <button
                  ...
                  className={`py-1.5 px-2 rounded-lg border text-xs font-semibold transition-all cursor-pointer text-center flex items-center justify-center ${
                    priority === 'high'
                      ? 'bg-stamp text-white border-stamp shadow-xs'
                      : 'bg-white text-slate-600 border-slate-200 hover:bg-slate-100'
                  }`}
                >
            ...
            <div>
              <label className="block font-semibold text-slate-800 mb-1.5 flex items-center">
                <Star className="w-3.5 h-3.5 mr-1 text-brass" />
                Interest Rating
              </label>
              <div className="flex items-center space-x-1 py-0.5">
                {[1, 2, 3, 4, 5].map((star) => (
                  <button
                    ...
                    className="p-1 text-slate-300 hover:text-brass focus:outline-hidden cursor-pointer transition-colors"
                  >
                    <Star
                      className={`w-5 h-5 ${star <= rating ? 'text-brass fill-brass' : ''}`}
                    />
                  </button>
                ))}
            ...
            <label className="block font-semibold text-slate-800 mb-1 flex items-center">
              ...
              <div className="flex items-center space-x-2">
                <input
                  ...
                  className="flex-1 px-3 py-1.5 bg-slate-50 border border-slate-200 rounded-lg text-slate-900 focus:outline-hidden focus:ring-2 focus:ring-ledger"
                />
                <button
                  type="button"
              ...
              {tags.length > 0 && (
                <div className="flex flex-wrap gap-1 mt-2">
                  {tags.map((t) => (
                    <span key={t} className="inline-flex items-center px-2 py-0.5 rounded bg-ledger-soft text-ledger border border-ledger text-[11px]">
                      #{t}
                      <button
                        type="button"
                        onClick={() => handleRemoveTag(t)}
                        className="ml-1 text-ledger hover:text-ledger cursor-pointer"
                      >
                        ✕
                      </button>
                    </span>
          ...
          <div className="bg-plum-soft/50 p-4 rounded-xl border border-plum-soft space-y-3">
            ...
            {interviewRounds.length > 0 && (
              <div className="space-y-2">
                {interviewRounds.map((round) => (
                  <div key={round.id} className="flex items-center justify-between bg-white p-2.5 rounded-lg border border-plum-soft shadow-2xs text-xs">
                    <div className="flex items-center space-x-2 min-w-0">
                      <button
                        type="button"
                        onClick={() => handleToggleRound(round.id)}
                        className="text-plum hover:text-plum cursor-pointer"
                      >
                        {round.completed ? (
                          <CheckCircle2 className="w-4 h-4 text-brass" />
                        ) : (
                          <Circle className="w-4 h-4 text-plum" />
                        )}
                      </button>
                    ...
                    <button
                      ...
                      className="text-slate-300 hover:text-stamp p-1 cursor-pointer"
                    >
            ...
            <div className="flex items-center space-x-2 pt-1">
              <input
                ...
                className="flex-1 px-3 py-1.5 bg-white border border-plum rounded-lg text-slate-900 focus:outline-hidden text-xs"
              />
              <input
                type="date"
                value={newRoundDate}
                onChange={(e) => setNewRoundDate(e.target.value)}
                className="px-2 py-1.5 bg-white border border-plum rounded-lg text-slate-900 text-xs"
              />
              <button
                type="button"
                onClick={handleAddInterviewRound}
                className="px-3 py-1.5 bg-plum hover:bg-plum text-white rounded-lg font-semibold text-xs cursor-pointer flex items-center"
              >
