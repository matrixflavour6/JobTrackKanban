...
export const KanbanBoard: React.FC<KanbanBoardProps> = ({
  ...
  const handleAdd = onAddJobForStage || onAddJob || (() => {});
        ...
        <button
          ...
          className={`px-3 py-1.5 rounded-lg text-xs font-semibold whitespace-nowrap cursor-pointer transition-all ${
            activeStageFilter === 'all'
              ...
              : 'bg-white text-ink-soft border border-ink/10 hover:bg-paper-dim'
          }`}
        >
          All 5 Columns
        </button>
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-medium whitespace-nowrap flex items-center space-x-1.5 cursor-pointer transition-all ${
                activeStageFilter === s.id
                  ? 'bg-ledger text-white shadow-xs font-semibold'
