              ...
              <th className="py-3.5 px-4 w-10">
                <button onClick={toggleSelectAll} className="cursor-pointer text-slate-400 hover:text-slate-700">
                  {selectedIds.length === applications.length && applications.length > 0 ? (
                    <CheckSquare className="w-4 h-4 text-ledger" />
                  ) : (
            ...
            {sortedApplications.length === 0 ? (
              ...
              sortedApplications.map((job) => {
                ...
                return (
                  <tr 
                    key={job.id} 
                    className={`hover:bg-slate-50/80 transition-colors ${isSelected ? 'bg-ledger-soft/30' : ''}`}
                  >
                      ...
                      <div className="flex items-center space-x-2">
                        ...
                        {job.jobUrl && (
                          <a 
                            ...
                            className="text-slate-400 hover:text-ledger"
                          >
                    ...
                    <td className="py-3 px-4 text-slate-600">
                      {job.followUpDate ? (
                        <div className="flex items-center space-x-1.5">
                          <span className={job.followUpDate <= new Date().toISOString().slice(0, 10) ? 'text-brass font-semibold' : ''}>
                            {job.followUpDate}
                          </span>
                          <button
                            onClick={() => downloadIcsFile(job)}
                            title="Download .ics Calendar File"
                            className="p-1 rounded hover:bg-slate-100 text-ledger transition-colors cursor-pointer"
                          >
                    ...
                    <td className="py-3 px-4 text-slate-500">
                      ...
                      {job.salary && <div className="text-brass font-medium text-[11px]">{job.salary}</div>}
                    </td>
                    ...
                    <td className="py-3 px-4">
                      <div className="flex flex-col space-y-1">
                        <div>
                          {job.priority === 'high' && (
                            <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-bold bg-stamp-soft text-stamp border border-stamp">
                              <Flame className="w-2.5 h-2.5 mr-0.5 fill-stamp text-stamp" />
                              High
                            </span>
                          )}
                          {job.priority === 'medium' && (
                            <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-semibold bg-brass-soft text-brass border border-brass">
                              Medium
                            </span>
                          )}
                          {job.priority === 'low' && (
                        ...
                        <div className="flex text-brass">
                          {[1, 2, 3, 4, 5].map((star) => (
                            <Star
                              key={star}
                              className={`w-3 h-3 ${star <= (job.rating || 0) ? 'fill-brass' : 'text-slate-200'}`}
                            />
                          ))}
                        </div>
                      </div>
                    ...
                    <td className="py-3 px-4 text-right">
                      <div className="flex items-center justify-end space-x-1">
                        ...
                        <button
                          ...
                          className="p-1.5 text-slate-400 hover:text-stamp hover:bg-stamp-soft rounded-md transition-colors cursor-pointer"
                          title="Delete"
                        >
