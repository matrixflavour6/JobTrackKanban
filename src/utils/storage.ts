...
export function loadApplications(): JobApplication[] {
  try {
    ...
    if (!saved) {
      // Genuinely nothing saved yet (first-ever visit) — seed demo data.
      saveApplications(INITIAL_SAMPLE_JOBS);
      return INITIAL_SAMPLE_JOBS;
    }
    const parsed = JSON.parse(saved);
    if (Array.isArray(parsed)) {
      // A real saved array — including a genuinely empty one, e.g. after
      // the person deletes every application on purpose. Returning it
      // as-is (rather than reseeding demo data whenever it's empty) is
      // what makes deletions actually stick across a refresh.
